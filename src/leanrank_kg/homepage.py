...
<main>
  ...
  <section>
    ...
    <div class="status-row" style="margin-top:12px">
      ...
      <div class="status ok"><div class="label">Label disjoint</div><div class="value">{{ production_evidence.supervision.all_positive_negative_pairs_disjoint|default(false) }}</div><div class="note">{{ "{:,}".format(production_evidence.supervision.total_positive_negative_overlap_removed|default(0) or 0) }} positive/negative overlaps removed</div></div>
      <div class="status ok"><div class="label">Train label overlap</div><div class="value">{{ "{:,}".format(production_evidence.supervision.train_positive_negative_pair_overlap_count|default(0) or 0) }}</div><div class="note">remaining positive/negative pair overlap after normalization</div></div>
...
function renderGuidance(data){
  ...
  const leanExtraction = query.lean_proof_state_extraction || {};
  const leanVariant = leanExtraction.source_variant ? `; Lean source: <b>${escapeHtml(leanExtraction.source_variant)}</b>` : '';
  const leanFallback = leanExtraction.fallback_reason ? `; fallback: <b>${escapeHtml(leanExtraction.fallback_reason)}</b>` : '';
