...
def test_audit_covers_refined_theorem_guidance_artifacts(tmp_path, monkeypatch):
    ...
    assert result["checks"]["validation:performance_optimization_plan"]["passed"] is True
    assert result["checks"]["validation:reproducibility_profile"]["passed"] is True
    assert result["checks"]["validation:retrieval_quality_profile"]["passed"] is True
