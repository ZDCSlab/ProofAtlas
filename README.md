...
`leanrank-kg build-research-report --config configs/proofatlas.yaml` writes `outputs/reports/research_report.md` and `outputs/predictions/research_prediction_results.json`. This is the compact research-facing deliverable: it frames ProofAtlas as a processed dataset plus retrieval-grounded prediction artifacts, and summarizes premise retrieval, proof-pattern retrieval, strategy-label retrieval from similar proof states, and difficulty-profile retrieval from historical neighbors without presenting the system as production proof automation.
